...
The tool SHALL clone repositories to `$XDG_CACHE_HOME/agentdeps/repos/` using `os.UserCacheDir()` for platform-appropriate cache paths.
...
- **THEN** the tool runs `git clone --branch <ref> --single-branch <url> <cache-dir>` to create a shallow single-branch clone
